...
- 2、配置读写分离操作；<br>
   - 2.1、SQL编写注意事项；<br>
- 3、配置sharding操作；<br>
   - 3.1 配置sql文件与逻辑代码结构；<br>
   - 3.2 一库一片算法；<br>
       - 3.2.1 片名连续的一库一片操作；<br>
       - 3.2.2 非片名连续的一库一片操作；<br>
   - 3.3 库内分片算法；<br>
       - 3.3.1 片名连续的库内分片操作；<br>
       - 3.3.2 非片名连续的库内分片操作；<br>
   - 3.4 自动生成数据源文件；<br>
...
## 2.1、SQL编写注意事项
sql语句的第一个参数必须是shardkey。耦合在业务代码中的sql语句,比如：insert into userinfo(uid,username) values(?,?)，这种写法是不支持的，约定写法必须为：insert into userinfo(uid,username) values("+ uid +",?)，也就是说，第一个参数不允许是占位符，而必须是实际参数。
