...
## 2.1、sql编写注意事项
sql语句的第一个参数必须是shardkey。耦合在业务代码中的sql语句,比如：insert into tab(column1,column2) values(?,?)，这种写法是不支持的，约定写法必须为：insert into tab(column1,column2) values("+ column1 +",?)，也就是说，第一个参数不允许是占位符，而必须是实际参数。
