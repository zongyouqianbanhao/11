...
使用配置中心后，程序中将不能够再继续使用Spring自动装配后的SharkJdbcTemplate，这是因为一旦配置中心发生改变，应用所持有的仍然是之前引用，因此必须通过com.sharksharding.core.shard.GetJdbcTemplate类的getSharkJdbcTemplate()方法获取SharkJdbcTemplate实例，如下所示：
```Java
